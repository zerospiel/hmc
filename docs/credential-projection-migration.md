...
*before* any KCM-managed services are reconciled, using a dedicated
`credsprojection.Projector`. Note that resources deployed by the cluster template
itself (e.g. k0s `Chart` manifests) are outside this ordering and may still race
projection.
...
| **Set** | Direct projection via `credsprojection.Projector`. The `ClusterDeployment` controller resolves the CAPI infrastructure provider, renders the template ConfigMap, and applies the results to the child cluster *before* reconciling KCM-managed services. Resources defined by the cluster template (e.g. k0s `Chart` manifests) are not gated by projection. |
...
|--------|-------------------|-------------------|
| Ordering | No guarantee - credentials and services race. | Credentials are projected before KCM-managed services are reconciled (reducing race risk); k0s `Chart` resources defined in cluster templates may still start before projection completes. |
...
1. Wait for the CAPI Cluster and child kubeconfig Secret to be available (and resolve the infrastructure provider object from the Cluster's `infrastructureRef`).
...
4. Set `CredentialsProjected=True` condition before reconciling KCM-managed services.
