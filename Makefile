...
SVELTOS_VERSION ?= $(shell \
	APP_VERSION=v$$(grep 'appVersion:' $(PROVIDER_TEMPLATES_DIR)/projectsveltos/Chart.yaml | cut -d '"' -f 2); \
	URL="https://raw.githubusercontent.com/projectsveltos/sveltos/$$APP_VERSION/manifest/crds/sveltos_crds.yaml"; \
	if curl -o /dev/null -Isf $$URL; then \
		echo "$$APP_VERSION"; \
	else \
		go mod edit -json | jq -r '.Require[] | select(.Path == "github.com/projectsveltos/libsveltos") | .Version'; \
	fi)
