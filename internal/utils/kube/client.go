...
func GetRegionalClientByRegionName(ctx context.Context, mgmtClient client.Client, systemNamespace, region string) (client.Client, error) {
	if region == "" {
	...
	rgnClient, _, err := GetRegionalClient(ctx, mgmtClient, systemNamespace, rgn)
	if err != nil {
