...
func (p *Projector) Project(ctx context.Context, cred *kcmv1.Credential, infraProvider *unstructured.Unstructured) error {
	...
	// CRD-level CEL enforces non-empty resourceTemplate, but not whitespace-only
	if cfg.ResourceTemplateRef == nil && strings.TrimSpace(cfg.ResourceTemplate) == "" {
		return errors.New("projectionConfig must set resourceTemplateRef or non-whitespace resourceTemplate")
	}
...
func (p *Projector) buildTemplateData(ctx context.Context, cred *kcmv1.Credential, infraProvider *unstructured.Unstructured) (*templateData, error) {
	...
	identityNS := ref.Namespace
	if identityNS == "" {
		identityNS = cred.Namespace
	}

	identityKey := client.ObjectKey{Namespace: identityNS, Name: ref.Name}
	...
	if cred.Spec.ProjectionConfig.SecretDataRef != nil {
