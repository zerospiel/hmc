...
func (t *SegmentIO) Collect(ctx context.Context) error {
	...
	mgmtData := newMgmtDataFetcher(true)
	...
	var (
		...
		mgmtID = string(mgmtData.mgmt.UID)
	)
...
func (*SegmentIO) collectChildProperties(ctx context.Context, childCl client.Client) (map[string]any, error) {
	...
	// populate nodes data
	props["node.count"] = acc.count
	props["node.cpu.total"] = acc.totalCPU
	props["node.memory.bytes"] = acc.totalMemory
	props["node.gpu.total"] = acc.totalGPUNodes
