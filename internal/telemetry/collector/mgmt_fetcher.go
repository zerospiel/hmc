...
func newMgmtDataFetcher(fetchMmgtCluster bool) *mgmtDataFetcher {
...
func (f *mgmtDataFetcher) fetch(ctx context.Context, mgmtClient client.Client) error {
	f.mgmt = new(kcmv1.Management)
	if f.fetchMgmtCluster {
		if err := f.retry(ctx, func() error {
			return mgmtClient.Get(ctx, client.ObjectKey{Name: kcmv1.ManagementName}, f.mgmt)
		}); err != nil {
